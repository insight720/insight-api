...
public class QuantityUsageStockDeductionListener implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt stockDeductionMessageExt) {
        ...
        boolean isDuplicate = RedisUtils.checkIdempotencyToken
                (redisTemplate, stockDeductionMessageKeysKey, orderSn);
        if (isDuplicate) {
            log.info("Duplicate stock deduction message, orderSn: {}", orderSn);
            return;
        }
        // 执行接口计数用法存量更新本地事务，并发送订单状态普通消息
        ...
        transactionTemplate.executeWithoutResult(ignored -> {
            registerTransactionSynchronization(orderStatusUpdateDTO, stockDeductionMessageKeysKey);
            facadeService.updateOrderQuantityUsageStock(stockDeductionDTO, orderStatusUpdateDTO);
            // 设置作用于计数用法订单状态更新普通消息消费过程幂等的订单号（Spring 管理的 Redis 事务）
     ...
     * 2. 回滚：回滚作用于接口计数用法存量扣减消息消费过程幂等的订单号。
     * 3. 未知：打印日志，人工处理。
     * </pre>
     *
                    ...
                    case STATUS_COMMITTED -> {
                        ...
                        try {
                            sendResult = rocketMQTemplate.syncSend
                                    (QUANTITY_USAGE_ORDER_STATUS_UPDATE_MESSAGE_DESTINATION, orderStatusUpdateDTO);
                            log.info("""
                    ...
                    case STATUS_ROLLED_BACK -> {
                        try {
                            ...
                            log.warn("""
                                    ...
                                    """, e.getMessage(), stockDeductionMessageKeysKey);
                        }
                    }
                    // 记录事务状态不确定的日志
                    case STATUS_UNKNOWN -> {
